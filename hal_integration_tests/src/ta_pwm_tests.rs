...
/// The `ta_pwm_test_runner` fixture reports over the backchannel at the
...
fn test_ta_pwm_self_check() -> Result<(), Box<dyn Error>> {
    deployment::build_and_flash("ta_pwm_test_runner")?;
    verify_self_check_burst()
}

/// Open the board's UART (8N1) and verify the fixed verdict burst the
/// `ta_pwm_test_runner` fixture transmits once per second.
